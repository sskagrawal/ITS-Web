...
public partial class Admin_OrganizationType : System.Web.UI.Page
{
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        ...
        {
            SqlCommand cmd = new SqlCommand("INSERT INTO [OrgnaizationType]([Description],[OrganizationName],[CreateOn]) VALUES (@Description,@OrganizationName,@CreateOn)");
            cmd.Parameters.AddWithValue("@Description",txtDes.Text);
            cmd.Parameters.AddWithValue("@OrganizationName", txtOrgaization.Text);
        ...
        {
            SqlCommand cmd = new SqlCommand("UPDATE [OrgnaizationType]  SET [Description] = @Description,[OrganizationName] = @OrganizationName WHERE ID=" + hdfID.Value + "");
            cmd.Parameters.AddWithValue("@Description", txtDes.Text);
            cmd.Parameters.AddWithValue("@OrganizationName", txtOrgaization.Text);
            DataAccess.InsertExecuteNonQuery(cmd);
            alert.Visible = true;
    ...
    protected void Edit(object sender, ImageClickEventArgs e)
    {
        ...
        if (btndetails.ID.Equals("imgbtnUpdate"))
        {
            ...
            DataTable dt = DataAccess.GetData("select * from [OrgnaizationType]  WHERE  ID =" + ID + "");
            if (dt.Rows.Count > 0 && dt != null)
            {
                txtDes.Text = dt.Rows[0]["Description"].ToString();
                txtOrgaization.Text = dt.Rows[0]["OrganizationName"].ToString();
        ...
        {
            SqlCommand cmd = new SqlCommand("DELETE FROM [OrgnaizationType]  WHERE  ID =" + ID + "");
            DataAccess.InsertExecuteNonQuery(cmd);
        }
        BindGrid();
    ...
    private void Clr()
    {
        ...
        txtOrgaization.Text = string.Empty;
