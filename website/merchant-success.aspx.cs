...
public partial class MerchantSuccess : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (Request.Params.Count > 0)
        {
            ...
            SqlCommand cmd = new SqlCommand("update [OnlinePaymentDetail] set [OrderStatus]=@OrderStatus,[FailureMessage]=@FailureMessage,[PaymentMode]=@PaymentMode,[StatusCode]=@StatusCode,[StatusMessage]=@StatusMessage,[TrackingId]=@TrackingId,TransactionDate=@TransactionDate where [OnlinePaymentID]=@OnlinePaymentID");
            DateTime ChkDate = DateTime.ParseExact(txn_date + txn_time, "dd/MM/yyyy HH:mm:ss", CultureInfo.InvariantCulture);
            cmd.Parameters.AddWithValue("@OrderStatus", status);
            cmd.Parameters.AddWithValue("@FailureMessage", res_message);
            cmd.Parameters.AddWithValue("@PaymentMode", paymode);
            cmd.Parameters.AddWithValue("@StatusCode", res_code);
            cmd.Parameters.AddWithValue("@StatusMessage", status);
            cmd.Parameters.AddWithValue("@TrackingId", ag_ref);
            cmd.Parameters.AddWithValue("@TransactionDate", ChkDate);
            cmd.Parameters.AddWithValue("@OnlinePaymentID", order_no);
