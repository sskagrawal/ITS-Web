               ...
               // cmd.Parameters.AddWithValue("@UserEmail", Request.Form["email"]);
                ...
                if (Request.Form["status"] == "success")
                {
                    if (merc_hash != Request.Form["hash"])
                    {
                        ...
                        //lblMsg.Text = "Hash value did not matched";
                        // btPrintInv.Visible = false;
                        Response.Redirect("~/PaymentFailure.aspx?trkId=" + DB_Configuration.Encryptdata(Request.Form["PayuMoneyId"]) + "&ordSt=" + DB_Configuration.Encryptdata(Request.Form["status"]));
                    }
                    else
                    {
                        Response.Redirect("~/PSuccess.aspx?trkId=" + DB_Configuration.Encryptdata(Request.Form["PayuMoneyId"]) + "&ordSt=" + DB_Configuration.Encryptdata(Request.Form["status"]));
                    }
                    }
